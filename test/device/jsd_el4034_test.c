...
#include "jsd/jsd_el4034_pub.h"
#include "jsd/jsd_el4034_types.h"
#include "jsd_test_utils.h"

extern FILE* file;
uint8_t      slave_id;
double       cmd_output[JSD_EL4034_NUM_CHANNELS] = {10.0, 10.0, 5.0, 0.0};

void telemetry_header() {
  if (!file) {
    return;
  }
  for (int i = 0; i < JSD_EL4034_NUM_CHANNELS; ++i) {
    fprintf(file, "EL4034_ch%d_raw_value, EL4034_ch%d_volts, ", i, i);
  }
  fprintf(file, "\n");
}
...
void telemetry_data(void* self) {
  ...
  const jsd_el4034_state_t* state = jsd_el4034_get_state(sds->jsd, slave_id);
...
void extract_data(void* self) {
  ...
  jsd_el4034_process(sds->jsd, slave_id);
}
...
void command(void* self) {
  ...
  // Zero values of channels every 5 seconds.

  if (iter % (sds->loop_rate_hz * 5) == 0) {
    flip = !flip;
    for (int i = 0; i < JSD_EL4034_NUM_CHANNELS; ++i) {
      if(flip){
        jsd_el4034_write_all_channels(sds->jsd, slave_id, cmd_output);
      } else {
        double zeros[JSD_EL4034_NUM_CHANNELS] = {0.0, 0.0, 0.0, 0.0};
        jsd_el4034_write_all_channels(sds->jsd, slave_id, zeros);
      }
    }
  }
...
int main(int argc, char const* argv[]) {
  if (argc != 4) {
    ...
    MSG("Usage: jsd_el4034_test <ifname> <el4034_slave_index> <loop_freq_hz>");
    MSG("Example: $ jsd_el4034 eth0 5 10");
    return 0;
  }
  ...
  my_config.product_code         = JSD_EL4034_PRODUCT_CODE;

  jsd_set_slave_config(sds.jsd, slave_id, my_config);

  sds_run(&sds, ifname, "/tmp/jsd_el4034.csv");
